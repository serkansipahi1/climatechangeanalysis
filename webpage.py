...
# Load the data
file_path = 'C:/Users/ABDURAHMAN BEY/PycharmProjects/pythonProject8/climatechangedata(1).xlsx'
...
def result():
    try:
        ...
        for ind in indicators_to_predict:
            variable = data[ind]
            train_data = data[(data.index.year <= 2023) & (data.index.year >= 1990)][ind]
            test_data = data[(data.index.year >= 2018) & (data.index.year <= 2023)][ind]
        ...
        return render_template('result.html', year=year, results=results)
    except Exception as e:
